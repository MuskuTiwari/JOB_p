...
route.get("/userdashboardview",checkUseAuth,FrontController.userdashboardview);
//user profile update 
...
route.post("/insertuser",[
    ...
    body('p').isLength({ min: 6 }).withMessage('Password must be at least 6 characters long'),
...
route.get("/category/admin/category/display",checkUseAuth,CategoryController.display);
route.get("/admin/category/view/:id",checkUseAuth,CategoryController.categoryview)
route.post("/admin/category/edit/:id",checkUseAuth,CategoryController.categoryedit)
...
route.post("/updateStatus/:id",checkUseAuth,CompanyJobController.updateStatus)
